...
export const Navbar: React.FC<NavbarProps> = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-gradient-to-r from-green-700 via-green-600 to-emerald-600 text-white shadow-lg backdrop-blur-md bg-opacity-95">
        ...
        <div className="flex items-center justify-between h-20">
          <div className="flex items-center space-x-3 cursor-pointer" onClick={() => setActiveTab('resident')}>
            ...
            <div>
              ...
              <p className="text-xs text-green-100 font-medium">Empowering Smart & Sustainable Cities</p>
            </div>
          </div>
          ...
          <nav className="hidden md:flex space-x-1 lg:space-x-2">
            {tabs.map((tab) => {
              ...
              return (
                ...
                >
                  <Icon className={`w-4 h-4 ${isActive ? 'text-green-700' : 'text-green-200'}`} />
                  {tab.label}
                </button>
              );
            })}
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs whitespace-nowrap font-medium transition-all ${
                  isActive
                    ...
                    : 'bg-green-800/40 text-green-100 hover:bg-white/10'
                }`}
              >
